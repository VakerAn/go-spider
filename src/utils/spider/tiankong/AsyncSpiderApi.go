...
func (spiderApi *AsyncSpiderApi) Start() {
	go StartAsyncApi()
}

func (spiderApi *AsyncSpiderApi) PageDetail(id string) {
	go Detail(id, 0)
}

func (spiderApi *AsyncSpiderApi) DoRecentUpdate() {
	DoRecentUpdate()
}
...
func DoRecentUpdate() {
	allMoviesDoneKeyExists := storage.RedisDB.Exists(context.TODO(), "all_movies_done").Val()
	recentUpdateKeyExists := storage.RedisDB.Exists(context.TODO(), "recent_update_key").Val()

	if allMoviesDoneKeyExists > 0 && recentUpdateKeyExists == 0 {
		startTime := time.Now()

		storage.RedisDB.SetNX(context.TODO(), "recent_update_key", "done", time.Second*3600).Err()
		...
		ExecMinutesS := strconv.FormatFloat(endTime.Minutes(), 'f', 2, 64)
		ExecHoursS := strconv.FormatFloat(endTime.Hours(), 'f', 2, 64)
		log.Println("最近更新执行完成，耗时：" + ExecSecondsS + "秒 \n 即" + ExecMinutesS + "分钟 \n 即" + ExecHoursS + "小时 \n " + runtime.GOOS)
	}
}
...
func actionRecentUpdateList() {
	...
	for _j := 1; _j <= pageCount; _j++ {
		...
		task := func() {
			...
			resp, gErr := pkghttp.IClient(pkghttp.Client).Get(&pkghttp.RequestForm{
				Uri: url,
			})
			if gErr != nil {
			...
			var nav ResData
			err := services.Json.Unmarshal(resp, &nav)
			if err != nil {
			...
			for _, value := range nav.List {
				if inType(value.TypeId, saveTypes) {
					...
					storage.RedisDB.ZAdd(context.TODO(), "detail_links:id:"+strconv.Itoa(value.TypeId), &redis.Z{
						Score:  float64(stamp1.Unix()),
						Member: `/?m=vod-detail-id-` + strconv.Itoa(value.VodId) + `.html`,
					})

					if inType(value.TypeId, film) {
						storage.RedisDB.ZAdd(context.TODO(), "detail_links:id:1", &redis.Z{
							Score:  float64(stamp1.Unix()),
							Member: `/?m=vod-detail-id-` + strconv.Itoa(value.VodId) + `.html`,
						})
						// 获取详情
					...
					if inType(value.TypeId, tv) {
						storage.RedisDB.ZAdd(context.TODO(), "detail_links:id:3", &redis.Z{
							Score:  float64(stamp1.Unix()),
							Member: `/?m=vod-detail-id-` + strconv.Itoa(value.VodId) + `.html`,
						})
						// 获取详情
					...
					if inType(value.TypeId, cartoon) {
						storage.RedisDB.ZAdd(context.TODO(), "detail_links:id:24", &redis.Z{
							Score:  float64(stamp1.Unix()),
							Member: `/?m=vod-detail-id-` + strconv.Itoa(value.VodId) + `.html`,
						})
						// 获取详情
...
func pageCount(subCategoryId string, retry int) (int, string) {
	...
	resp, err := pkghttp.IClient(pkghttp.Client).Get(&pkghttp.RequestForm{
		Uri: url,
	})
	if err != nil {
		retry++
	...
	var nav ResData
	jErr := services.Json.Unmarshal(resp, &nav)
	if jErr != nil {
...
func Detail(id string, retry int) {
	...
	kuyunAryJson, _ := services.Json.MarshalIndent(kuyunAry, "", " ")
	ckm3u8AryJson, _ := services.Json.MarshalIndent(ckm3u8Ary, "", " ")
	downloadAryJson, _ := services.Json.MarshalIndent(downloadAry, "", " ")
	...
	_detail, _ := services.Json.MarshalIndent(mDetail, "", " ")

	_moviesInfo["detail"] = string(_detail)

	t := storage.RedisDB.HMSet(context.TODO(), "movies_detail:"+link+":movie_name:"+listDetail.VodName, _moviesInfo).Err()
...
func subCategoryIds() []string {
	...
	if storage.RedisDB.Exists(context.TODO(), "categories").Val() == 0 {
		storage.RedisDB.Set(context.TODO(), "categories", categoriesStr, 0).Err()
	}

	CategoryIds := make([]string, 0)

	err := services.Json.Unmarshal([]byte(categoriesStr), &nav)
	if err != nil {
...
func DelAllListCacheKey() {
	storage.RedisDB.Del(context.TODO(), "paginate")
}
